...
async def _run_compare(run_id: str, req: CompareRequest):
    ...
    try:
        ...
        async def _replay_both(recording_id: str):
            ...
            async with sem:
                ...
                async with sf() as db2:
                    recording = await db2.get(Recording, recording_id)
                if not recording:
                    return
                ...
                # Diff each against original recording
                ...
                return status_a == status_b  # agreed?

        results = await asyncio.gather(
            *[_replay_both(rid) for rid in recording_ids],
        ...
        # Fix Bug 3: mark run as FAILED on unhandled exception
        print(f"[compare] _run_compare failed for run {run_id}: {e}")
        try:
            async with async_session_factory() as db:
                run = await db.get(CompareRun, run_id)
